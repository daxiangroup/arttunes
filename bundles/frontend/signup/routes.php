...
Route::get('(:bundle)', function() {
    // Get the form data
    $form_data = Signup\Services\Signup::form(\Session::get('error_object'))
        ->get();
...
Route::post('(:bundle)', function() {
    ...
    if ($validation->fails()) {
        return \Redirect::to('/signup')
            ...
            ->with('error_object', $validation->errors->messages);
    }
...
Route::get('(:bundle)/creator', function() {
    // Get the form data
    $form_data = Signup\Services\SignupCreator::form()
        ->get();
